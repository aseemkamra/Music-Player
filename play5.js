...
// Fetches the list of songs from the "play5" directory
async function getsongs() {
    try {
        const response = await fetch('http://127.0.0.1:5500/play5/');
        ...
        for (let i = 0; i < anchors.length; i++) {
            ...
            if (anchor.href.endsWith(".mp3")) {
                songs.push(anchor.href.split("/play5/")[1]);
            }
        }
        return songs;
    } catch (error) {
...
async function main() {
    ...
    if (savedSongSrc) {
        try {
            ...
            if (savedSongSrc.includes("/play5/")) {
                trackName = decodeURIComponent(savedSongSrc.split("/play5/")[1]);
